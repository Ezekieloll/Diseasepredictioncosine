...
def normalize_text(text):
    """ Converts text to lowercase, replaces underscores, removes extra spaces. """
...
def correct_symptoms(user_symptoms):
    """ Matches user input symptoms to closest correct symptoms in the dataset. Filters out garbage values. """
    corrected_symptoms = []
...
def predict_disease(user_symptoms):
    ...
    user_input_text = ' '.join(user_symptoms).strip()
...
user_input = [normalize_text(symptom) for symptom in user_input]

# 🔹 Correct & filter symptoms (remove garbage values)
valid_symptoms = correct_symptoms(user_input)

# 🔹 If no valid symptoms, show an error message
if not valid_symptoms:
    print("\n⚠️ Error: No valid symptoms detected. Please enter correct symptoms.")
else:
    # 🔹 Predict disease
